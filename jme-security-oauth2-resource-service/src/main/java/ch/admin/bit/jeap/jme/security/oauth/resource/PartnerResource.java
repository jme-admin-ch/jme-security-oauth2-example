...
public class PartnerResource {
     ...
     */
    @GetMapping
    @PreAuthorize("hasRole('partner_read')")
    public Collection<Partner> listPartners() {
        // Does the token grant read access on all partners?
        if (jeapAuthorization.hasRoleForAllPartners("partner_read")) {
            // Fetch all partners.
            return listAll();
        }
        else {
            // Determine the partner ids the token grants read access on...
            Collection<String> partners = jeapAuthorization.getPartnersForRole("partner_read");
    ...
    @PreAuthorize("hasRoleForPartner('partner_read', #partnerId)")
    ...
    @PreAuthorize("hasRole('partner_read')")
    @PostAuthorize("hasRoleForPartner('partner_read', returnObject.getId())")
    ...
    public String getPartnerNameByExternalRef(@PathVariable("externalRef") String externalRef) {
        ...
        if (jeapAuthorization.hasRoleForPartner("partner_read", partner.getId())) {
